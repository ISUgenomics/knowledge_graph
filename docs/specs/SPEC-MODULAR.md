...
+-- config/default.yaml            User config (created on first run)
+-- pyproject.toml
```
...
**`config/default.yaml` (created on first run, user-editable):**

```yaml
# Knowledge Graph Explorer configuration

db:
  path: ../runtime_data/vault/vault.db    # repo-local SQLite database
...
- [ ] Create default `config/default.yaml` generation on first run

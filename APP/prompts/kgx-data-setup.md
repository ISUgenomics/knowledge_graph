...
You are an expert at designing knowledge graphs for visualization. Your job is to help a user convert their source data into a KGX-compatible SQLite database and write the optimal `config.yaml` to maximize the usefulness of the explore mode UI.
...
### Phase 3: Configure — Tune explore mode for optimal UX
...
10. **Write the config.yaml** — Generate the full config file tuned for this dataset.
11. **Verify end-to-end** — Launch KGX, confirm the graph in explore mode shows meaningful clusters, not a hairball.
...
The `explore:` section of config.yaml controls server-side graph transformations that simplify the raw graph into a meaningful exploration view. Each transformation is optional — leave a field empty (`""` or `[]`) to skip it.
...
explore:
  ...
  # Hide intermediate entity types that add clutter. Their relationships are
  # preserved transitively through collaboration synthesis.
  exclude_types: []          # e.g. ["publication"] or ["lcr_type"]

  # --- Collaboration synthesis ---
  # Create weighted edges between entities that co-occur on the same intermediate type.
  # Example: two researchers who co-authored papers get a COLLABORATOR edge.
  collaboration_via_type: "" # The intermediate entity type (e.g. "publication")
  collaboration_via_edge: "" # The rel type connecting entities to intermediates (e.g. "AUTHORED")
  collaboration_label: COLLABORATOR  # Name for the synthesized edge
  ...
  tagging_edge: ""           # Rel type connecting entities to tags (e.g. "TAGGED")

  # --- Relationship skipping ---
  # Hide raw rel types that are replaced by synthetic ones above.
  skip_rel_types: []         # e.g. ["AUTHORED", "COAUTHOR", "BROADER"]
```
...
2. **Is there an intermediate type that connects primaries?** → Use `exclude_types` + `collaboration_via_*` to collapse it into direct edges
3. **Do you have a tag/category hierarchy?** → Use `hierarchy_edge` + `tagging_edge`
4. **Which raw rel types become redundant after synthesis?** → Add to `skip_rel_types`
...
python -m kgx --config config.yaml
```
...
explore:
  stub_type: ""
  exclude_types: [lcr_type]
  collaboration_via_type: lcr_type
  collaboration_via_edge: HAS_LCR
  collaboration_label: SIMILAR_COMPOSITION
  hierarchy_edge: ""
  tagging_edge: ""
...
explore:
  ...
  tagging_edge: TAGGED       # enables transitive tag connections
```

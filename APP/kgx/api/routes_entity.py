...
def make_entity_router(db: KnowledgeGraphDB, explore_config: dict | None = None) -> APIRouter:
    router = APIRouter(tags=["entities"])
    _explore_cfg = explore_config or {}

    @router.get("/entities/{entity_type}")
    def get_entities(entity_type: str, search: str = "", limit: int = 500, core_only: bool = False):
        """
    ...
    def get_entity(entity_id: str):
        ...
        Uses explore-aware queries so hierarchy parents can surface
        transitive annotation neighbors.
        """
        entity = db.get_entity(entity_id)
        if not entity:
            raise HTTPException(status_code=404, detail=f"Entity not found: {entity_id}")

        hierarchy_edge = _explore_cfg.get("hierarchy_edge", "BROADER")
        annotation_edge = _explore_cfg.get("annotation_edge", "TAGGED")
        neighbors = db.neighbors_explore(
            entity_id,

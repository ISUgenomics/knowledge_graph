...
def make_layout_router(
    ...
    _ui_cfg = ui_config or {}
    _db_build_cfg = db_build_config or {}

    @router.get("/layout/timeline/options")
    def get_timeline_options():
        timeline_cfg = ((_ui_cfg.get("layouts") or {}).get("timeline")) or None
        timeline_contract = ((_db_build_cfg.get("visualization") or {}).get("timeline")) or None
        detected_min_count = (
            (timeline_cfg or {}).get("detection", {}).get("detected_type_min_count", 1)
        )
        candidates = db.timeline_candidates(min_type_count=detected_min_count)
        return {
            "profile": timeline_cfg,
            "contract": timeline_contract,
            "candidates": candidates,
            "detected_type_min_count": detected_min_count,
        }

    @router.get("/layout/hierarchy/options")
    def get_hierarchy_options():
        hierarchy_cfg = ((_ui_cfg.get("layouts") or {}).get("hierarchical")) or None
        hierarchy_contract = ((_db_build_cfg.get("visualization") or {}).get("hierarchical")) or None
        return {
            "profile": hierarchy_cfg,

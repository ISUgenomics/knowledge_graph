...
Entity types: person, publication, signal, event, center, tag, award, organization
Relationship types: AUTHORED, ATTENDED, MENTIONED_IN, TAGGED, COAUTHOR, MEMBER_OF, WON, AFFILIATED_WITH
...
ENTITY_TYPES = {"person", "publication", "signal", "event", "center", "tag", "award", "organization", "acknowledgement"}
...
RELATIONSHIP_TYPES = {
    ...
    "TAGGED",        # any → tag
    "ACKNOWLEDGES",  # publication → organization/tag acknowledged in supporting text
    "ACKNOWLEDGED",  # publication -> acknowledgement evidence node
    "CREDITED",      # acknowledgement -> organization/tag that was acknowledged
    "COAUTHOR",      # person ↔ person (undirected)
    "COLLABORATOR",  # profiled person ↔ profiled person (peer relation)
...
class VaultDB:
    ...
    def _init_schema(self):
        ...
        """)
        # Keep builder-created DBs aligned with the current KGX schema version.
    ...
    def clear_acknowledgements(self, publication_id: str):
        """Remove acknowledgement relationships, evidence nodes, and snippets for a publication."""
        publication_id = self._normalize_id(publication_id)
        ack_ids = [
            row["target_id"]
            for row in self.conn.execute(
                "SELECT target_id FROM relationships WHERE source_id = ? AND rel_type IN ('ACKNOWLEDGED', 'HAS_ACKNOWLEDGEMENT')",
                (publication_id,),
            ).fetchall()
        ]
        self.conn.execute(
            "DELETE FROM relationships WHERE source_id = ? AND rel_type = 'ACKNOWLEDGES'",
            (publication_id,),
        )
        self.conn.execute(
            "DELETE FROM relationships WHERE source_id = ? AND rel_type = 'ACKNOWLEDGED'",
            (publication_id,),
        )
        self.conn.execute(
            "DELETE FROM relationships WHERE source_id = ? AND rel_type = 'HAS_ACKNOWLEDGEMENT'",
            (publication_id,),
        )
        self.conn.execute(
            "DELETE FROM snippets WHERE entity_id = ? AND ref_type = 'acknowledgement'",
            (publication_id,),
        )
        for ack_id in ack_ids:
            self.delete_entity(ack_id)
        self.conn.commit()
    ...
    def add_acknowledgement(
        ...
        *,
        ...
        """Record acknowledgement evidence for a publication.

        This is intentionally additive and lossless:
        - evidence node: acknowledgement
        - relationship: publication -> acknowledgement
        - relationship: acknowledgement -> acknowledged entity
        - optional snippet attached to the acknowledgement node
        - optional source provenance for the acknowledgement node
        """
        publication_id = self._normalize_id(publication_id)
        target_id = self.resolve(target_id) or self._normalize_id(target_id)
        ack_target = self.get_entity(target_id)
        target_name = (ack_target or {}).get("name", target_id)
        target_meta = (ack_target or {}).get("metadata", {}) or {}
        target_title = target_meta.get("description") or target_name
        publication = self.get_entity(publication_id) or {"metadata": {}, "name": publication_id}
        pub_meta = publication.get("metadata", {}) or {}
        journal = str(pub_meta.get("journal", "") or "").strip()
        year = str(pub_meta.get("year", "") or "").strip()
        signature_parts = [
            publication_id,
        ...
        label_parts = [part for part in [journal, year] if part]
        if label_parts:
            ack_name = f"Acknowledgement {' '.join(label_parts)}"
        else:
            ack_name = f"Acknowledgement {ordinal + 1}"
        ack_id = self._normalize_id(
            f"acknowledgement:{publication_id}:{digest}"
        )
        ack_meta = {
        ...
        self.add_relationship(publication_id, "ACKNOWLEDGED", ack_id, metadata=metadata)
        self.add_relationship(ack_id, "CREDITED", target_id, metadata=metadata)
        if text.strip():
            existing = [
                snippet for snippet in self.get_snippets(ack_id)
                if snippet.get("text", "").strip() == text.strip()
            ]
            if existing:
                text = ""
            else:
    ...
    def add_coauthor_relationship(self, person_a_id: str, person_b_id: str):
        """Add a person-person relationship with stable orientation.

        Policy:
        - if both sides are profiled, store `COLLABORATOR`
        - if exactly one side is profiled, store `profiled -> stub`
        - otherwise fall back to canonical lexical ordering
        """
        person_a_id = self.resolve(person_a_id) or self._normalize_id(person_a_id)
        person_b_id = self.resolve(person_b_id) or self._normalize_id(person_b_id)
        if person_a_id == person_b_id:
            return

        entity_a = self.get_entity(person_a_id)
        entity_b = self.get_entity(person_b_id)
        profiled_a = bool((entity_a or {}).get("metadata", {}).get("profiled"))
        profiled_b = bool((entity_b or {}).get("metadata", {}).get("profiled"))

        if profiled_a and profiled_b:
            source_id, target_id = tuple(sorted([person_a_id, person_b_id]))
            self.add_relationship(source_id, "COLLABORATOR", target_id)
            return
        if profiled_a != profiled_b:
            source_id, target_id = (
                (person_a_id, person_b_id) if profiled_a else (person_b_id, person_a_id)
            )
        else:
            source_id, target_id = tuple(sorted([person_a_id, person_b_id]))

        self.add_relationship(source_id, "COAUTHOR", target_id)

    def clear_profiled_person_links(self, entity_id: str) -> set[str]:
        """Remove relationships managed by person-profile materialization.

        Returns the set of publications previously authored by the profiled person so
        callers can prune stale publication/coauthor subgraphs after re-materializing.
        """
        entity_id = self._normalize_id(entity_id)
        old_pub_ids = {
            row["target_id"]
            for row in self.conn.execute(
                "SELECT target_id FROM relationships WHERE source_id = ? AND rel_type = 'AUTHORED'",
                (entity_id,),
            ).fetchall()
        }
        ...
        self.conn.execute(
            ...
            WHERE source_id = ?
              AND rel_type IN ('TAGGED', 'AUTHORED', 'WON', 'AFFILIATED_WITH', 'MEMBER_OF')
            """,
            (entity_id,),
        )
        self.conn.execute(
            """
            DELETE FROM relationships
            WHERE rel_type IN ('COAUTHOR', 'COLLABORATOR')
    ...
    def prune_publications_without_profiled_authors(self, publication_ids: set[str]):
        """Drop stale publication subgraphs that no longer have profiled authors."""
        for pub_id in sorted(self._normalize_id(pid) for pid in publication_ids if pid):
            author_ids = [
                row["source_id"]
                for row in self.conn.execute(
                    "SELECT source_id FROM relationships WHERE target_id = ? AND rel_type = 'AUTHORED'",
                    (pub_id,),
                ).fetchall()
            ]
            if not author_ids:
                self.conn.execute("DELETE FROM entities WHERE id = ?", (pub_id,))
                continue

            profiled_authors = []
            for author_id in author_ids:
                entity = self.get_entity(author_id)
                if entity and entity["type"] == "person" and entity["metadata"].get("profiled"):
                    profiled_authors.append(author_id)

            if profiled_authors:
                continue

            self.conn.execute(
                "DELETE FROM relationships WHERE target_id = ? AND rel_type = 'AUTHORED'",
                (pub_id,),
            )
            self.conn.execute(
                "DELETE FROM relationships WHERE source_id = ? AND rel_type = 'TAGGED'",
                (pub_id,),
            )
            self.conn.execute("DELETE FROM entities WHERE id = ?", (pub_id,))
